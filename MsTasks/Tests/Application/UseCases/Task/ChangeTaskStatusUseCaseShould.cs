...
public sealed class ChangeTaskStatusUseCaseShould
{
    [Fact]
    public async System.Threading.Tasks.Task ExecuteAsync_DelegatesToTaskService()
    {
        ...
        var expected = new TaskModel(GIVEN_TASK_ID, "Task", null, 1, null, TaskPriority.Medium, TaskStatus.InProgress, null, GIVEN_USER_ID, 0, DateTime.UtcNow, DateTime.UtcNow);

        var serviceMock = new Mock<ITaskService>();
        serviceMock
            .Setup(x => x.ChangeStatusAsync(GIVEN_TASK_ID, request, GIVEN_USER_ID, It.IsAny<CancellationToken>()))
            .ReturnsAsync(expected);

        var useCase = new ChangeTaskStatusUseCase(serviceMock.Object);
